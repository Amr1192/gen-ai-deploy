...
export default function CVAnalysisProPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Persist original CV text for Enhance CV page (if available)
      try {
        const raw = (result as any).originalText
        if (typeof raw === "string" && raw.trim()) {
          localStorage.setItem("cv_from_analysis", raw)
        }
      } catch {}

      if (file.type === "application/pdf" || file.type === "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
      // Store file metadata for enhance page
      localStorage.setItem("cv_file_name", file.name)
      localStorage.setItem("cv_file_type", file.type)
    }
